...
class Maze { //Based on "Depth First Search Algorithm" (DFS)
    static OBSTACLE = -1;
    static PASSABLE = 1;
    ...
    initNodes() {
        for (var x = 0; x < this.nodes.length; x++) {
            for (var y = 0; y < this.nodes[x].length; y++) {
                this.nodes[x][y] = new Node(x, y, null, Maze.OBSTACLE); //-1 = obstacle
            }
        }
    }
    ...
    generateSegment(x, y, currentNode) { //return grid a la fin !!
        ...
        if (x + 2 < this.sizeX && this.nodes[x + 2][y].id == Maze.OBSTACLE) {
            directions.push(1)
        }
        if (x - 2 >= 0 && this.nodes[x - 2][y].id == Maze.OBSTACLE) {
            directions.push(2) //Ajout des directions possible
        }
        if (y + 2 < this.sizeY && this.nodes[x][y + 2].id == Maze.OBSTACLE) {
            directions.push(3)
        }
        if (y - 2 >= 0 && this.nodes[x][y - 2].id == Maze.OBSTACLE) {
            directions.push(4)
        }
    ...
    createSegment(x, y, dx, dy, currentNode) {
        this.nodes[x + dx][y + dy].id = Maze.PASSABLE;
        this.nodes[x + 2 * dx][y + 2 * dy].id = Maze.PASSABLE;
